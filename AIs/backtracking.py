...
def preprocessing(mazeMap, mazeWidth, mazeHeight, playerLocation, opponentLocation, piecesOfCheese, timeAllowed):
    ...
    newPieces = backtrackTSP(mazeMap, piecesOfCheese, playerLocation)[0]
...
def turn(mazeMap, mazeWidth, mazeHeight, playerLocation, opponentLocation, playerScore, opponentScore, piecesOfCheese, timeAllowed):
    global heatMap
    global cheeseMap
    lastPieces = [piece for piece in newPieces if piece in piecesOfCheese]
    t0 = time.time()
    lestMouvements = A_to_all(mazeMap, playerLocation, lastPieces)
    # print('time atoall'+str(time.time()-t0))
    heatMap[(mazeHeight-playerLocation[1]-1, playerLocation[0])] += 1
    return FIFO_pop(lestMouvements)
