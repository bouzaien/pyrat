...
def preprocessing(mazeMap, mazeWidth, mazeHeight, playerLocation, opponentLocation, piecesOfCheese, timeAllowed):
    global visitedCells
    global VisitedCellsCount
    visitedCells = [playerLocation]
    VisitedCellsCount[playerLocation] = 1
...
def turn(mazeMap, mazeWidth, mazeHeight, playerLocation, opponentLocation, playerScore, opponentScore, piecesOfCheese, timeAllowed):
        ...
        if len(listDiscoveryMoves(mazeMap, currentPosition)) == 0:
            ...
            m = moveFromLocation(currentPosition, minChoice(listDiscoveryMoves(mazeMap, currentPosition), VisitedCellsCount))
        visitedCells.append(step(currentPosition, m))
        if step(currentPosition, m) in VisitedCellsCount:
            VisitedCellsCount[step(currentPosition, m)] += 1
        else:
            VisitedCellsCount[step(currentPosition, m)] = 1
        return m            
...
def minChoice(possibleList, visitedDict):
    extractedVisitedDict = {position:visitedDict[position] for position in possibleList}
    finalList = [position for position in extractedVisitedDict if extractedVisitedDict[position] == min(list(extractedVisitedDict.keys()))]
